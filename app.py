# GTM Global Trade & Logistics Dashboard — Sri Lanka Focus (Aligned + Safe column selection)
# - Live UN Comtrade+ (imports/exports) by HS-6
# - FX (USD→LKR) with override
# - Landed cost calc + shocks; multi-origin compare
# - Route mapping (folium + Nominatim)
# - Presets, tariff helper, packing calculator
# - No sidebar; top control ribbon; alignment fixes
...
.topwrap { display:grid; grid-template-columns: 1.6fr 1fr 1fr .9fr; gap:.6rem; align-items:end }
.topwrap2{ display:grid; grid-template-columns: 1fr 1fr 1fr .8fr .6fr; gap:.6rem; align-items:end; margin-top:.6rem }
...
.hstack { display:grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap:.6rem }
.equal { display:grid; grid-template-columns: 1.1fr .9fr; gap:.6rem }
...
def pick_col(df: pd.DataFrame, names, fill=None):
    """Return the first existing column in names; otherwise a Series filled with fill or empty Series."""
    for n in names:
        if n in df.columns:
            return df[n]
    if fill is None:
...
st.caption("Live trade • FX • Landed cost • Routes • Packing • Presets")
...
# row 1
c1, c2, c3, c4 = st.columns([1.6, 1, 1, .9])
with c1:
...
with c4:
    ...
    years = st.selectbox("years",
        ["2019,2020,2021,2022,2023","2020,2021,2022,2023,2024","2018,2019,2020,2021,2022"],
        index=0, key="w_years", label_visibility="collapsed")

# row 2
c5, c6, c7, c8, c9 = st.columns([1, 1, 1, .8, .6])
with c5:
...
partners = (ndf.groupby("partner")["value_usd"].sum()
...
with right:
    ...
    if st.session_state.get("btn_route"):
        o = geocode_point(origin_q); d = geocode_point(dest_q)
        if o and d:
            a, b = (o[0], o[1]), (d[0], d[1])
        ...
        st.caption("Click “Plot / Update Route” to render the map.")
    st.markdown("</div>", unsafe_allow_html=True)
...
st.markdown("<div class='equal'>", unsafe_allow_html=True)

# Presets + Tariff
st.markdown("<div class='card'>", unsafe_allow_html=True)
st.markdown("**Presets & Tariff Helper**")
preset_name = st.selectbox("Preset", list(PRESETS.keys()), index=0, key="w_preset")
...
if not partners.empty:
    for _, r in partners.head(10).iterrows(): row[f"partner_{r.partner}"] = float(r.value_usd)
